...
# --- IMPORTAÇÕES EXTERNAS (Assumindo que tens isto no teu projeto) ---
# Se não tiveres, mantém as funções auxiliares que definiste antes do código da classe
...
class Experiment:
    ...
    def _train_eval_model(self, model, X_train, y_train, X_val, y_val, epochs=None):
        ...
        if epochs is None or not hasattr(model, "partial_fit"):
            ...
            if X_val is not None and y_val is not None:
                ...
                return metrics["F1"], model, None # Sem histórico
    ...
    def run_tvt_with_feature_selection(self, val_size=0.2, test_size=0.2, 
        ...
        for name, model_original in self.models.items():
            ...
            print(f"   > A testar número de features (k=1 a {num_features})...")
            ...
            print(f"   🏁 Teste Final: F1={metrics_test['F1']:.4f}")
